...
def main():
    ...
    y = get_time_matrix((len(all_times), len(combinations)), all_times)
    y = [ torch.from_numpy(y[i, :]).type(torch.float32) for i in range(y.shape[0])]
    ...
    extraction_function = lambda x: torch.min(x, -1)[1].cpu()
    ...
    print(majority_index, [np.sum(new_y[:, i]) for i in range(len(combinations))])
    ...
    print(f"train set:\nvb:{sum(min_train)}     pred:{sum(pred_train)}      sb:{sum(majority_train)}")
    print(f"validation set:\nvb:{sum(min_val)}     pred:{sum(pred_val)}     sb:{sum(majority_val)}")
    print(f"test set:\nvb:{sum(min_test)}     pred:{sum(pred_test)}     sb:{sum(majority_test)}")
